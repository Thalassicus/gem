...
UPDATE Units SET	   Combat = ROUND(1.00 * (SELECT Combat FROM Units WHERE Type = 'UNIT_CANNON'), 0)
					   WHERE Type IN ('UNIT_KOREAN_HWACHA');
UPDATE Units SET RangedCombat = ROUND(1.00 * (SELECT RangedCombat FROM Units WHERE Type = 'UNIT_CANNON'), 0)
					   WHERE Type IN ('UNIT_KOREAN_HWACHA');

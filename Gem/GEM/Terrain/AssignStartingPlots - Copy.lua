...
------------------------------------------------------------------------------
function AssignStartingPlots:PlaceStrategicResourceInPlots(plotList, resID, resAmount, maxDepositSize)
	local iW, iH = Map.GetGridSize()
	local depositSize = math.min(resAmount, 1 + Map.Rand(maxDepositSize, "Strategic Resource Placement"))
	for _, plotID in ipairs(plotList) do
		...
		if plot:GetResourceType(-1) == -1 then
			...
			resAmount = resAmount - depositSize;
...
function AssignStartingPlots:GetStrategicDistributionForPlots(resID, plotList)
	local resAmount			= GameInfo.Resources[resID].NumPerTerritory
	...
	resAmount = resAmount * stratMultiplier
	if resAmount < 0 then
		resAmount = -resAmount * (numPlots / Civup.PLOTS_PER_CITY)
	end
	maxDepositSize = math.ceil(resAmount * 80 / numPlots)
	
	return Game.Round(resAmount), maxDepositSize
end
------------------------------------------------------------------------------
function AssignStartingPlots:PlaceStrategicAndBonusResources()
	...
	for regionID, regionInfo in ipairs(self.regionData) do
		...
		for _, resID in pairs(resIDs) do
			local resAmount, maxDepositSize = self:GetStrategicDistributionForPlots(resID, plotList[resID])
			local passes = 0
			----log:Debug("Placing %2s %9s in Region %2s", resAmount, Locale.ConvertTextKey(GameInfo.Resources[resID].Description), regionID)
			while resAmount > 0 and passes < Civup.MAX_RESOURCE_PLACEMENT_ATTEMPTS do
				passes = passes + 1
				table.sort(plotList[resID], function (a,b)
					return self.plotResInfo[resID][a].weightAvg > self.plotResInfo[resID][b].weightAvg
				end)				
				resAmount = self:PlaceStrategicResourceInPlots(plotList[resID], resID, resAmount, maxDepositSize)
			end
			
			if resAmount > 0 then
				log:Error("Unable to place %i %s in region %i after %i passes.", resAmount, GameInfo.Resources[resID].Type, regionID, passes)
			end
		end
	end

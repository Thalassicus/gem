...
--[[
local maxVal = math.max

local GetMin = function(q, distanceTable)
	local m = maxVal
	local i = 0
	for k, v in pairs(q) do
		if distanceTable[v] < m then
			m = distanceTable[v]
			i = k
		end
	end
...
function dijkstra (graph, start)
	...
	local distanceTable = table.fill(maxVal, numPlots)
	local previousTable = {}
	for i = 1, numPlots do
		distanceTable[i] = maxVal
	end
	distanceTable[start] = 0
	local S = {}
	local Q = {}
	for v = 1, numPlots do Q[v] = v end -- fill Q
	local nQ = numPlots -- # elements in Q
	while nQ > 0 do
		local u = GetMin(Q, distanceTable)
		nQ = nQ - 1
		table.insert(S, u)
		for v = 1, numPlots do
			if G[u][v] > 0 -- edge between u and v?
					and distanceTable[v] > distanceTable[u] + G[u][v] then -- relax
				distanceTable[v] = distanceTable[u] + G[u][v]
				previousTable[v] = u
			end
		end
	end
	return distanceTable, previousTable
end
...
function AssignStartingPlots:AssignCityStatesToRegionsOrToUninhabited(args)
	...
	-- Debug check
	if self.iNumCityStatesUnassigned ~= 0 then
		print("ERROR: Wrong number of City States assigned at end of assignment process. This number unassigned: ", self.iNumCityStatesUnassigned);
	else
